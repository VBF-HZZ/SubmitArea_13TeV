...
def parseOptions():
    ...
    parser.add_option('-f', '--fragment', dest='FRAGMENT', type='string',default='', help='GEN-SIM fragment')
...
def submitLHEGENSIM():
    ...
    gensimfragment = opt.FRAGMENT
    if (gensimfragment==''):
        print 'No GEN-SIM fragment specified!!!'
        sys.exit()
    ...
    outDir='resultsLHEGENSIM_'+tag 

    if (not os.path.isdir(outDir)):
        cmd = 'mkdir '+outDir
        processCmd(cmd)

    outFilesDir = 'outFilesLHEGENSIM_'+tag
    if not os.path.isdir(outFilesDir):
        cmd = 'mkdir '+outFilesDir
        processCmd(cmd)

    errFilesDir = 'errFilesLHEGENSIM_'+tag
    ...
    for job in range(1,njobs+1):
        ...
        jobName = 'jobLHEGENSIM_'+str(int(job))             
        cmd = 'qsub -o '+outFilesDir+'/'+jobName+'.out -e '+errFilesDir+'/'+jobName+'.err -v jobName='+jobName+',curDir='+currentDir+',outDir='+outDir+',gridpack='+gridpack+',job='+str(int(job))+',eventsperjob='+str(eventsperjob)+',gensimfragment='+gensimfragment+',tag='+opt.TAG+' -N \"'+jobName+'\" submitFileLHEGENSIM.pbs.sh' 
        #print cmd 
